...
class AudioManager:
    def __init__(self):
        ...
        self.music_fallback_channel = None
    ...
    def _init_mixer(self):
        try:
            ...
            self.sfx["click"] = self._load_sound(
                ...
                create_tone(freq=760, duration=0.05, volume=0.22, wave="sine"),
            )
            self.sfx["hit"] = self._load_sound(
                SOUNDS_DIR / "hit.wav",
                create_tone(freq=145, duration=0.18, volume=0.34, wave="square"),
            )
            self.sfx["success"] = self._load_sound(
                SOUNDS_DIR / "success.wav",
                create_tone(freq=960, duration=0.16, volume=0.30, wave="sine"),
            )

            # Dedicated emotional cues requested for Mind Maze.
            self.sfx["get_ready"] = self._load_first_available(
                [SOUNDS_DIR / "get_ready.wav", SOUNDS_DIR / "start.wav", SOUNDS_DIR / "click.wav"],
                create_tone(freq=540, duration=0.22, volume=0.30, wave="sine"),
            )
            self.sfx["oh_no"] = self._load_first_available(
                [SOUNDS_DIR / "oh_no.wav", SOUNDS_DIR / "hit.wav"],
                create_tone(freq=118, duration=0.30, volume=0.36, wave="square"),
            )
            self.sfx["energetic_win"] = self._load_first_available(
                [SOUNDS_DIR / "energetic_win.wav", SOUNDS_DIR / "success.wav"],
                create_tone(freq=1120, duration=0.26, volume=0.34, wave="saw"),
            )
    ...
    def _load_sound(self, path, fallback_sound):
        if path.exists():
            try:
    ...
    def _load_first_available(self, paths, fallback_sound):
        for path in paths:
            if path.exists():
                try:
                    return pygame.mixer.Sound(str(path))
    ...
    def play_sfx(self, key, volume_scale=1.0):
        ...
        if not sound:
            return
        sound.set_volume(clamp(self._sfx_volume_for(key) * volume_scale, 0.0, 1.0))
        sound.play()
    ...
    def adjust_music_volume(self, delta):
        self.music_master = clamp(self.music_master + delta, 0.0, 1.0)
        self.apply_music_volume()
        return self.music_master

    def adjust_sfx_volume(self, delta):
        self.sfx_master = clamp(self.sfx_master + delta, 0.0, 1.0)
        return self.sfx_master

    def shutdown(self):
        if not self.enabled:
            return
        self._stop_current_music(fade_ms=180)
...
class MindMazeGame:
    ...
    def play_sfx(self, name, volume_scale=1.0):
        self.audio.play_sfx(name, volume_scale=volume_scale)

    def set_audio_status(self, message):
        self.audio_status_message = message
    ...
    def try_move(self, dr, dc):
        ...
        if self.is_wall(nr, nc):
            ...
            self.play_sfx("oh_no", volume_scale=1.0)
        ...
        self.trail_stamp_timer = 0.0
        self.play_sfx("click", volume_scale=0.55)
    ...
    def activate_focus_mode(self):
        ...
        self.play_sfx("click", volume_scale=0.75)
    ...
    def handle_start_events(self, event):
        ...
        if self.start_btn.handle_event(event):
            self.play_sfx("get_ready", volume_scale=1.0)
            self.start_new_run()
    ...
    def handle_gameplay_events(self, event):
        ...
        if event.key in (pygame.K_UP, pygame.K_w):
            ...
            self.play_sfx("click", volume_scale=0.8)
        elif event.key in (pygame.K_f, pygame.K_SPACE):
            self.activate_focus_mode()

    def handle_audio_shortcuts(self, event):
        if event.type != pygame.KEYDOWN:
            return

        if event.key == pygame.K_m:
            muted = self.audio.toggle_mute()
            self.set_audio_status("Audio Muted" if muted else "Audio Unmuted")
            return

        if event.key in (pygame.K_MINUS, pygame.K_KP_MINUS):
            vol = self.audio.adjust_music_volume(-0.05)
            self.set_audio_status(f"Music Volume: {int(vol * 100)}%")
        elif event.key in (pygame.K_EQUALS, pygame.K_KP_PLUS):
            vol = self.audio.adjust_music_volume(0.05)
            self.set_audio_status(f"Music Volume: {int(vol * 100)}%")
        elif event.key == pygame.K_COMMA:
            vol = self.audio.adjust_sfx_volume(-0.05)
            self.set_audio_status(f"SFX Volume: {int(vol * 100)}%")
    ...
    def handle_complete_events(self, event):
        ...
        if self.continue_btn.handle_event(event):
            self.play_sfx("click", volume_scale=1.0)
            ...
            self.play_sfx("click", volume_scale=0.95)
            self.transition_to(STATE_START)
    ...
    def resolve_level_complete(self):
        ...
        self.play_sfx("energetic_win", volume_scale=1.0)
    ...
    def resolve_game_over(self):
        ...
        self.play_sfx("oh_no", volume_scale=0.8)
    ...
    def update_effects(self, dt):
        ...
        self.audio_status_timer = max(0.0, self.audio_status_timer - dt)
    ...
    def draw_hud(self, surface):
        ...
        audio_state = "MUTED" if self.audio.muted else f"MUS {int(self.audio.music_master * 100)}%  SFX {int(self.audio.sfx_master * 100)}%"
        audio_hint = f"Audio: {audio_state}   [M mute | -/+ music | ,/. sfx]"
        draw_glow_text(
            surface,
            audio_hint,
            self.tiny_font,
            (bottom_rect.x + 18, bottom_rect.bottom - 18),
            (176, 208, 238),
            NEON_BLUE,
        )

        if self.audio_status_timer > 0:
            draw_glow_text(
                surface,
                self.audio_status_message,
                self.small_font,
                (WIDTH // 2, HEIGHT - BOTTOM_BAR_H - 12),
                TEXT_COLOR,
                NEON_GREEN,
                center=True,
            )
    ...
    def run(self):
        while True:
            ...
            for event in pygame.event.get():
                ...
                self.handle_audio_shortcuts(event)
